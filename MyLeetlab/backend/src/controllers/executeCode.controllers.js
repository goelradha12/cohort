...
import { SUBMISSION_STATUS, outputsMatch } from "../libs/judgeUtils.js";
...
export const executeCode = asyncHandler(async function (req, res) {
    ...
    try {
        ...
        const detailedResults = results.map((result, index) => {
            ...
            const passed = outputsMatch(stdout, expectedOutput);
            if (!passed) {
                allPassed = false;
            }
        ...
        // Persist submission, solved marker, and testcase results atomically so a
        // mid-write failure cannot leave partial data (remediation item 3).
        const { submissionDoc, testcaseResults } = await db.$transaction(async (tx) => {
            const createdSubmission = await tx.Submission.create({
                data: {
                    ...
                    status: allPassed ? SUBMISSION_STATUS.ACCEPTED : SUBMISSION_STATUS.WRONG_ANSWER,
                    stdin: stdin.join("\n"),
            ...
            const preparedTestcaseResults = detailedResults.map((result) => {
                return {
            ...
            await tx.TestCaseResult.createMany({
                data: preparedTestcaseResults
            })

            return { submissionDoc: createdSubmission, testcaseResults: preparedTestcaseResults }
        })
        ...
        console.error("Code execution failed", { userId: req.user?._id, problemId: req.body?.problemId, languageId: req.body?.language_id, name: error.name, message: error.message, code: error.code, statusCode: error.statusCode, stack: error.stack })
...
export const runCode = asyncHandler(async function(req, res){
    // Goal: Run the code and send back the result without saving it
    try {
        const {source_code, language_id, stdin, expected_outputs, problemId} = req.body;
        ...
        return res.status(200).json(
            new apiResponse(
                ...
                {
                    submission: submissionDoc,
                    TestCaseResult: detailedResults
                }
                ,
                "Code Executed Successfully"
            )
        ...
        console.error("Code run failed", { userId: req.user?._id, problemId: req.body?.problemId, languageId: req.body?.language_id, name: error.name, message: error.message, code: error.code, statusCode: error.statusCode, stack: error.stack })
        if (error instanceof apiError) {
            return res.status(error.statusCode).json({
                statusCode: error.statusCode,
                message: error.message,
