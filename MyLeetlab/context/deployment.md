...
## Repository evidence
The app is containerized: `backend/Dockerfile` (Node 20, `npm ci`, `prisma generate`, runs `node src/index.js`), `frontend/Dockerfile` (multi-stage Vite build → nginx serving `dist/` with SPA fallback via `frontend/nginx.conf`), root `docker-compose.yml` wiring both services, and `DOCKER.md` with usage. The database is external (managed PostgreSQL / Neon) — no DB container. `.dockerignore` files keep `node_modules`, `.env*`, and the generated Prisma client out of build contexts. No CI workflow or cloud hosting manifest yet. See `DOCKER.md`.
...
- Frontend `VITE_API_URL` (build-time) points at the backend subdomain; compose default set accordingly.
